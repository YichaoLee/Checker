...
class InterPCFGTraits:public DefaultDOTGraphTraits{	
 //Copy and modified  from CFGPrinter.h
public:
...
public:
	CFGWriter(ProgramCFG *CFG1,raw_ostream& out,bool isSimple = false):
	G(CFG1),O(out),DTraits(isSimple){}
...
/*bool writeGraph(){
  ...
  void writeGraph1(const std::string &Title,CFG* cfg){
   writeHeader(Title);
...
// Writes the edge labels of the node to O and returns true if there are any
  ...
  void writeNodes1(CFG* cfg) {
    // Loop over the graph, printing it out...
    for(unsigned int i=0;i<cfg->stateList.size();i++)
    for (node_iterator I = G->nodes->begin(), E = G->nodes->end() ;
         I != E; ++I) 
        if(I->second.id == cfg->stateList[i].ID)
        //writeNode(I->second);
        writeNode1(I->second,cfg);
  }
  ...
  void writeNode1(CFGNode& Node,CFG * cfg) {
    writeNode1(&Node,cfg);
  }

  void writeNode1(CFGNode *const *Node,CFG* cfg) {
    writeNode1(*Node,cfg);
  }
  ...
  void writeNode1(CFGNode *Node,CFG * cfg) { 
    ...
    O<<"\n"<<"\t"<<cfg->stateList[SN++].ContentRec;//cfg->ContentRec;
    std::string edgeSourceLabels;
    raw_string_ostream EdgeSourceLabels(edgeSourceLabels);
    bool hasEdgeSourceLabels = getEdgeSourceLabels(EdgeSourceLabels, Node);
